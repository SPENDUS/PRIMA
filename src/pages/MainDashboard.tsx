...
export default function MainDashboard({ user, onLogout, onNavigate, darkMode, toggleDarkMode }: { user: any, onLogout: () => void, onNavigate: (page: string) => void, darkMode: boolean, toggleDarkMode: () => void }) {
  ...
  const [KasihIbuIcon, setKasihIbuIcon] = useState<any>(HeartHandshake);
          ...
          if (data.success && data.data) {
            ...
            if (data.data.kasih_ibu_icon) {
              import('lucide-react').then(LucideIcons => {
                const IconComponent = (LucideIcons as any)[data.data.kasih_ibu_icon] || HeartHandshake;
  ...
  const menuItems = [
    ...
    { id: 'garda', url: 'https://gardaspendus.netlify.app/', img: 'https://lh3.googleusercontent.com/d/1ltGZoLoeamrE79q-Uzwx3KUg6A987qo2', label: 'GARDA', gradient: 'from-indigo-400 to-indigo-600', shadow: 'shadow-indigo-500/40', roles: ['guru'] },
    ...
    { id: 'game_generator', url: 'https://gamegenerator.ai.studio/', icon: Gamepad2, label: 'Game Generator', gradient: 'from-yellow-400 to-orange-500', shadow: 'shadow-orange-500/40', roles: ['guru'] },
  ];
  ...
  const unfilledSchedules = user?.role === 'guru' && stats?.todaysScheduleBoxes ? stats.todaysScheduleBoxes.filter((sch: any) => !sch.status) : [];
  const hasUnfilledSchedule = unfilledSchedules.length > 0;
            ...
            {user?.role === 'guru' && (
              <div className="relative">
                <button 
                  ...
                  className="bg-white/10 hover:bg-white/20 text-white p-2 rounded-lg transition-colors relative"
                >
                  <Bell className="w-5 h-5" />
                  {hasUnfilledSchedule && (
                    <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-transparent animate-pulse shadow-sm"></span>
                  )}
                </button>
                <AnimatePresence>
                  {showNotifications && (
                    <motion.div 
                      ...
                      className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50"
                    >
                      ...
                      <div className="max-h-64 overflow-y-auto p-2">
                        {hasUnfilledSchedule ? (
                          <div className="space-y-2">
                            <p className="text-xs text-red-500 font-medium px-2 py-1 bg-red-50 dark:bg-red-900/20 rounded-lg">
                              Ada jadwal yang belum diisi Jurnal hari ini:
                            </p>
                            {unfilledSchedules.map((sch: any, idx: number) => (
                              <button 
                                key={idx}
                                onClick={() => {
                                ...
                                className="w-full text-left p-3 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-600 group"
                              >
                                <p className="font-bold text-slate-800 dark:text-white text-sm group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{sch.mapel}</p>
                                <p className="text-xs text-slate-500 mt-1">Kelas {sch.kelas} • Jam ke-{sch.jam}</p>
                              </button>
                            ))}
                          </div>
                        ) : (
          ...
          {activeTab === 'profil' && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6 border border-slate-100 dark:border-slate-700 animate-in fade-in duration-300">
              ...
              <div className="space-y-4">
                ...
                { (user?.role === 'guru' || user?.role === 'tendik') && (
                  <button 
