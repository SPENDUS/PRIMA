...
async function run() {
  const { data: murid } = await supabase.from('murid').select('Kelas').limit(5);
  console.log("Murid sample:", murid);
  
  const { data: uniqueKelas } = await supabase.from('murid').select('Kelas');
  const set = new Set(uniqueKelas?.map(m => m.Kelas));
  console.log("Unique kelas in murid:", Array.from(set));
}
run();
