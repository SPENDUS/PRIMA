...
try {
  if (fs.existsSync(configPath)) {
    ...
    if (localConfig.supabaseUrl) currentSupabaseUrl = localConfig.supabaseUrl;
    if (localConfig.supabaseKey) currentSupabaseKey = localConfig.supabaseKey;
  }
} catch (e) {
  console.error("Error loading config.json", e);
}

let supabase = createClient(currentSupabaseUrl, currentSupabaseKey);
...
app.get('/api/admin/stats', async (req, res) => {
  ...
  app.post('/api/db-config', (req, res) => {
    try {
      ...
      if (supabaseUrl && supabaseKey) {
        ...
        res.status(400).json({ success: false, message: 'URL and Key are required' });
      }
    } catch (e: any) {
      res.status(500).json({ success: false, message: e.message });
    }
  ...
  app.get('/api/kasih-ibu-config', async (req, res) => {
    try {
      const { data, error } = await supabase.from('pengaturan').select('key, value').in('key', ['kasih_ibu_name', 'kasih_ibu_icon']);
      if (error) throw error;
